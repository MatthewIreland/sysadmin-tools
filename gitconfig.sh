...
git config --global user.email "matthew.t.ireland@gmail.com"

git config credential.helper 'cache'                         # avoid having to type in credentials for github every push/pull
git config credential.helper 'cache --timeout=100000'        # max value for timeout (seconds)
...
# git config --global comit.template $HOME/.gitmessage.txt     # sets the default message that appears in the editor upon commit
